...
cdef class ComputeSimilarityCython:
    ...
    def __init__(self, data_matrix, top_k=100, shrink=0, normalize=True,
        ...
        self.n_columns = data_matrix.shape[1]
        self.n_rows = data_matrix.shape[0]
        self.shrink = shrink
        ...
        if similarity == 'adjusted':
            ...
            raise ValueError('Cosine_Similarity: value for parameter "mode" not recognized. Allowed values are: "cosine", "pearson", "adjusted", "asymmetric", "jaccard", "tanimoto", "dice", "tversky". Passed value was "{}"'.format(similarity))
        ...
        if self.adjusted_cosine:
            data_matrix = self.apply_adjusted_cosine(data_matrix)
        elif self.pearson_correlation:
            data_matrix = self.apply_pearson_correlation(data_matrix)
        elif self.tanimoto_coefficient or self.dice_coefficient or self.tversky_coefficient:
            data_matrix = self.use_only_boolean_interactions(data_matrix)
        # Compute sum of squared values to be used in normalization
        self.sum_of_squared = np.array(data_matrix.power(2).sum(axis=0), dtype=np.float64).ravel()
        ...
        if self.asymmetric_cosine:
            self.sum_of_squared_to_1_minus_alpha = np.power(self.sum_of_squared, 2 * (1 - self.asymmetric_alpha))
            self.sum_of_squared_to_alpha = np.power(self.sum_of_squared, 2 * self.asymmetric_alpha)
        # Apply weight after sum_of_squared has been computed but before the matrix is split in its inner data structures
        self.use_row_weights = False
        if row_weights is not None:
            if data_matrix.shape[0] != len(row_weights):
                raise ValueError('Cosine_Similarity: provided row_weights and data_matrix have different number of rows. row_weights has {} rows, data_matrix has {}.'.format(len(row_weights), data_matrix.shape[0]))
            self.use_row_weights = True
            self.row_weights = np.array(row_weights, dtype=np.float64)
        data_matrix = check_matrix(data_matrix, 'csr')
        self.user_to_item_row_ptr = data_matrix.indptr
        self.user_to_item_cols = data_matrix.indices
        self.user_to_item_data = np.array(data_matrix.data, dtype=np.float64)
        data_matrix = check_matrix(data_matrix, 'csc')
        self.item_to_user_rows = data_matrix.indices
        self.item_to_user_col_ptr = data_matrix.indptr
        self.item_to_user_data = np.array(data_matrix.data, dtype=np.float64)
        if self.top_k == 0:
            self.W_dense = np.zeros((self.n_columns,self.n_columns))

    cdef use_only_boolean_interactions(self, data_matrix):
        """
        Set to 1 all data points
        :return:
        """
        cdef long index
        for index in range(len(data_matrix.data)):
            data_matrix.data[index] = 1
        return data_matrix

    cdef apply_pearson_correlation(self, data_matrix):
        """
        Remove from every data point the average for the corresponding column
        :return:
        """
        cdef double[:] sum_per_col
        cdef int[:] interactions_per_col
        cdef long col_index, inner_index, start_pos, end_pos
        cdef double col_average
        data_matrix = check_matrix(data_matrix, 'csc')
        sum_per_col = np.array(data_matrix.sum(axis=0), dtype=np.float64).ravel()
        interactions_per_col = np.diff(data_matrix.indptr)
        # Remove for every row the corresponding average
        for col_index in range(self.n_columns):
            if interactions_per_col[col_index] > 0:
                col_average = sum_per_col[col_index] / interactions_per_col[col_index]
                start_pos = data_matrix.indptr[col_index]
                end_pos = data_matrix.indptr[col_index + 1]
                inner_index = start_pos
                while inner_index < end_pos:
                    data_matrix.data[inner_index] -= col_average
                    inner_index += 1
        return data_matrix

    cdef apply_adjusted_cosine(self, data_matrix):
        """
        Remove from every data point the average for the corresponding row
        :return:
        """
        cdef double[:] sum_per_row
        cdef int[:] interactions_per_row
        cdef long row_index, inner_index, start_pos, end_pos
        cdef double row_average
        data_matrix = check_matrix(data_matrix, 'csr')
        sum_per_row = np.array(data_matrix.sum(axis=1), dtype=np.float64).ravel()
        interactions_per_row = np.diff(data_matrix.indptr)
        # Remove for every row the corresponding average
        for row_index in range(self.n_rows):
            if interactions_per_row[row_index] > 0:
                row_average = sum_per_row[row_index] / interactions_per_row[row_index]
                start_pos = data_matrix.indptr[row_index]
                end_pos = data_matrix.indptr[row_index + 1]
                inner_index = start_pos
                while inner_index < end_pos:
                    data_matrix.data[inner_index] -= row_average
                    inner_index += 1
        return data_matrix

    cdef int[:] get_users_that_rated_item(self, long item_id):
        return self.item_to_user_rows[self.item_to_user_col_ptr[item_id]:self.item_to_user_col_ptr[item_id+1]]

    cdef int[:] get_items_rated_by_user(self, long user_id):
        return self.user_to_item_cols[self.user_to_item_row_ptr[user_id]:self.user_to_item_row_ptr[user_id+1]]

    cdef compute_item_similarities(self, long item_id_input):
        """
        ...
        cdef int[:] users_that_rated_item = self.get_users_that_rated_item(item_id_input)
        ...
        for user_index in range(len(users_that_rated_item)):
            ...
            items_rated_by_user = self.get_items_rated_by_user(user_id)
    ...
    def compute_similarity(self, start_col=None, end_col=None):
        ...
        cdef int item_index, innerItemIndex, item_id, local_top_k
        cdef long long top_k_item_index
        ...
        cdef double[:] values = np.zeros((self.n_columns*self.top_k))
        cdef int[:] rows = np.zeros((self.n_columns*self.top_k,), dtype=np.int32)
        cdef int[:] cols = np.zeros((self.n_columns*self.top_k,), dtype=np.int32)
        ...
        while item_index < end_col_local:
            ...
            self.compute_item_similarities(item_index)
            # Apply normalization and shrinkage, ensure denominator != 0
            if self.normalize:
                for inner_item_index in range(self.n_columns):
                    if self.asymmetric_cosine:
                        self.this_item_weights[inner_item_index] /= self.sum_of_squared_to_alpha[item_index] * self.sum_of_squared_to_1_minus_alpha[inner_item_index] + self.shrink + 1e-6
                    else:
                        self.this_item_weights[inner_item_index] /= self.sum_of_squared[item_index] * self.sum_of_squared[inner_item_index] + self.shrink + 1e-6
            # Apply the specific denominator for Tanimoto
            elif self.tanimoto_coefficient:
                for inner_item_index in range(self.n_columns):
                    self.this_item_weights[inner_item_index] /= self.sum_of_squared[item_index] + self.sum_of_squared[inner_item_index] - self.this_item_weights[inner_item_index] + self.shrink + 1e-6
            elif self.dice_coefficient:
                for inner_item_index in range(self.n_columns):
                    self.this_item_weights[inner_item_index] /= self.sum_of_squared[item_index] + self.sum_of_squared[inner_item_index] + self.shrink + 1e-6
            elif self.tversky_coefficient:
                for inner_item_index in range(self.n_columns):
                    self.this_item_weights[inner_item_index] /= self.this_item_weights[inner_item_index] + (self.sum_of_squared[item_index] - self.this_item_weights[inner_item_index]) * self.tversky_alpha + (self.sum_of_squared[inner_item_index] - self.this_item_weights[inner_item_index]) * self.tversky_beta + self.shrink + 1e-6
            elif self.shrink != 0:
                for inner_item_index in range(self.n_columns):
            ...
            if self.top_k == 0:
                ...
                local_top_k = min([self.top_k, self.this_item_weights_counter])
                # Get the unordered set of top-k items
                top_k_partition = np.argpartition(this_item_weights_np, local_top_k-1)[0:local_top_k]
                ...
                for inner_item_index in range(len(top_k_idx)):
                    top_k_item_index = top_k_idx[inner_item_index]
                    item_id = self.this_item_weights_id[top_k_item_index]
            ...
            if processed_items % print_block_size == 0 or processed_items == end_col_local:
                ...
                if current_time - start_time != 0:
                    item_per_sec = processed_items / (current_time - start_time)
                else:
                    item_per_sec = 1
                print_block_size = int(item_per_sec * 30)
                if current_time - last_print_time > 30  or processed_items == end_col_local:
                    print('similarity column {} ( {:2.0f} % ), {:.2f} column/sec, elapsed time {:.2f} min'.format(processed_items, processed_items * 1.0 / (end_col_local - start_col_local) * 100, item_per_sec, (time.time() - start_time) / 60))
