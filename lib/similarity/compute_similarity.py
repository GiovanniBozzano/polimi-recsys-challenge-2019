...
class ComputeSimilarity:
    ...
    def __init__(self, dataMatrix, use_implementation = "density", similarity = None, **args):
        ...
        assert np.all(np.isfinite(dataMatrix.data)), \
            "Compute Similarity: Data matrix contains {} non finite values".format(np.sum(np.logical_not(np.isfinite(dataMatrix.data))))
        self.dense = False
        if similarity == "euclidean":
            ...
            assert not (dataMatrix.shape[0] == 1 and dataMatrix.nnz == dataMatrix.shape[1]),\
                "Compute Similarity: data has only 1 feature (shape: {}) with dense values," \
            ...
            if use_implementation == "density":
                if isinstance(dataMatrix, np.ndarray):
                    ...
                    print("Compute Similarity: matrix type not recognized, calling default...")
                    use_implementation = "python"
                if self.dense:
                    print("Compute Similarity: detected dense matrix")
                    use_implementation = "python"
                else:
            ...
            if use_implementation == "cython":
                try:
                    ...
                    print("Unable to load cython Compute Similarity, reverting to Python")
                    self.compute_similarity_object = ComputeSimilarityPython(dataMatrix, **args)
            elif use_implementation == "python":
                self.compute_similarity_object = ComputeSimilarityPython(dataMatrix, **args)
            else:
                raise ValueError("Compute Similarity: value for argument 'use_implementation' not recognized")
