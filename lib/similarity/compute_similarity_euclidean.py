...
class ComputeSimilarityEuclidean:
    ...
    def __init__(self, data_matrix, top_k=100, shrink=0, normalize=False, normalize_avg_row=False,
        ...
        if row_weights is not None:
            ...
            if data_matrix.shape[0] != len(row_weights):
                raise ValueError(
                    ...
                    'row_weights has {} rows, dataMatrix has {}.'.format(len(row_weights), data_matrix.shape[0]))

            self.use_row_weights = True
            self.row_weights = row_weights.copy()
            self.row_weights_diag = sps.diags(self.row_weights)

            self.dataMatrix_weighted = self.data_matrix.T.dot(self.row_weights_diag).T
    ...
    def compute_similarity(self, start_col=None, end_col=None, block_size=100):
        ...
        item_distance_initial = np.array(self.data_matrix.power(2).sum(axis=0)).ravel()
        sum_of_squared = np.sqrt(item_distance_initial)
        ...
        while start_col_block < end_col_local:
            ...
            for col_index_in_block in range(this_block_size):
                ...
                item_similarity[column_index] = 0.0
