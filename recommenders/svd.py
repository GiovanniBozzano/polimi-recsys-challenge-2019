...
class SVD(object):
    ...
    def __init__(self, n_factors=2000, knn=100):
        self.n_factors = n_factors
        self.knn = knn
        self.urm_training = None

    def fit(self, urm_training):
        self.urm_training = urm_training
        ...
        if icm_file.is_file():
            ...
            icm = URMUtils.get_icm()
            self.S_ICM_SVD = self.get_S_ICM_SVD(icm, k=self.n_factors, knn=self.knn)
            np.save('../data/icm_svd.npz.npy', self.S_ICM_SVD)

    def get_expected_ratings(self, playlist_id):
        playlist_id = int(playlist_id)
        liked_tracks = self.urm_training[playlist_id]
    ...
    def recommend(self, playlist_id, at=10):
        ...
        unseen_items_mask = np.in1d(recommended_items, self.urm_training[playlist_id].indices,
